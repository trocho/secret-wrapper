...
require_installed_skill() {
  root="$1"
  skill_name="$2"
  skill_path="$(find "$root" -type f -path "*/${plugin}/*/skills/${skill_name}/SKILL.md" -print -quit)"
  test -n "$skill_path" || fail "installed ${skill_name} skill was not found below $root"
